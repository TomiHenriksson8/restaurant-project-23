...
const removeFavoriteRestaurant = (id: string) => {
  ...
  const favoriteRestaurants: Restaurant[] = JSON.parse(favoriteRestaurantsData);
...
const restaurantModal = (restaurant: Restaurant, menu: any, type: MenuType) => {
  ...
  if (type === "daily" && menu.courses) {
      menu.courses.forEach((course: any) => {
...
const displayFavoriteRestaurants = (): void => {
  const favoriteRestaurantsData = localStorage.getItem("favoriteRestaurants");
  const favoritesSection = document.getElementById("favorites-section") as HTMLElement | null;

  if (favoritesSection) favoritesSection.innerHTML = ""; 
